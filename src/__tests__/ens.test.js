...
describe('Blockchain tests', () => {
  ...
  describe('Registry', () => {
    test('getOwner returns owner', async () => {
      const accounts = await getAccounts()
      const owner = await ens.getOwner('resolver.one')
      expect(owner).toBe(accounts[0])
    })

    test('setSubnodeOwner sets new subnode owner', async () => {
      const ensContract = ens.getENSContractInstance()
      const owner = await ensContract.owner(getNamehash('subnode.resolver.one'))
      const accounts = await getAccounts()
      expect(owner).toBe('0x0000000000000000000000000000000000000000')
      const tx = await ens.setSubnodeOwner('subnode.resolver.one', accounts[0])
      await tx.wait()
    ...
    test('setNewOwner sets new owner', async () => {
      ...
      const tx2 = await ens.setOwner('givethisaway.awesome.one', accounts[1])
      await tx2.wait()
      const newOwner = await ensContract.owner(hash)
      expect(newOwner).toBe(accounts[1])
    })
    ...
    test('getResolver returns a resolver address when set', async () => {
      const resolver = await ens.getResolver('resolver.one')
    ...
    test('setResolver sets the resolver on a node', async () => {
      //test setResolver
      const resolver = await ens.getResolver('awesome.one')
      const mockResolver = '0x0000000000000000000000000000000000abcdef'
      expect(resolver).not.toBe(mockResolver)

      const tx = await ens.setResolver('awesome.one', mockResolver)
      await tx.wait()
      const newResolver = await ens.getResolver('awesome.one')
    ...
    test('getTTL returns a TTL', async () => {
      const ttl = await ens.getTTL('resolver.one')
    ...
    test('createSubdomain makes a new subdomain', async () => {
      ...
      const tx = await ens.createSubdomain('new.resolver.one')
      await tx.wait()
      const newOwner = await ensContract.owner(hash)
    ...
    test('deleteSubdomain deletes a subdomain', async () => {
      ...
      // expect the initial owner to be no one
      expect(oldOwner).toBe('0x0000000000000000000000000000000000000000')
      const tx = await ens.createSubdomain('b.subdomain.one')
      await tx.wait()
      const newOwner = await ensContract.owner(hash)
      // Verify owner is the user and therefore the subdomain exists
      expect(newOwner).toBe(accounts[0])
      const tx2 = await ens.deleteSubdomain('b.subdomain.one')
      await tx2.wait()
  ...
  describe('Resolver', () => {
    test('getAddress returns an address', async () => {
      const addr = await ens.getAddress('resolver.one')
      expect(addr).toBeHex()
    ...
    test('getAddress returns 0x000', async () => {
      const tx = await ens.createSubdomain('addr.testing.one')
      await tx.wait()
      const resolverAddr = await ens.getAddress('resolver.one')
      const tx2 = await ens.setResolver('addr.testing.one', resolverAddr)
      await tx2.wait()
      const addr = await ens.getAddress('addr.testing.one')
      expect(addr).toBe('0x0000000000000000000000000000000000000000')
    })

    test('getAddr returns an eth address', async () => {
      const addr = await ens.getAddress('resolver.one', 'ONE')
      expect(addr).toBeHex()
      expect(addr).toBeEthAddress()
      expect(addr).not.toBe('0x0000000000000000000000000000000000000000')
    })
    ...
    test('setAddress sets an address', async () => {
      ...
      const resolverAddr = await ens.getAddress('resolver.one')
      const tx = await ens.setResolver('superawesome.one', resolverAddr)
      await tx.wait()
      ...
      await tx2.wait()
      const addr = await ens.getAddress('superawesome.one')
    ...
    test('setAddr sets an eth address', async () => {
      ...
      await tx2.wait()
      const addr = await ens.getAddr('superawesome.one', 'ONE')
      expect(addr).toBe('0x0000000000000000000000000000000000012345')
    })

    test('getContent returns a 32 byte hash', async () => {
      const content = await ens.getContent('oldresolver.one')
    ...
    test('setContentHash sets up ipfs has', async () => {
      const contentHash =
        'ipfs://QmTeW79w7QQ6Npa3b1d5tANreCDxF2iDaAPsDvW6KtLmfB'
      await ens.setContenthash('abittooawesome.one', contentHash)

      const content = await ens.getContent('abittooawesome.one')
  ...
  describe('Reverse Registrar', () => {
    ...
    test('claimAndSetReverseRecordName claims and sets a name', async () => {
      ...
      const tx = await ens.claimAndSetReverseRecordName('resolver.one', 2000000)
      await tx.wait()
  ...
  describe('Helper functions', () => {
    test('getDomainDetails gets rootdomain and resolver details', async () => {
      try {
        const domain = await ens.getDomainDetails('resolver.one')
    ...
    test('getSubdomains gets all subdomains', async () => {
      const domains = await ens.getSubdomains('one')
